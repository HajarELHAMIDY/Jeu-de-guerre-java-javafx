...
public class MonstreF extends GraphicObject {

	private Circle sortie = new Circle(0, 0, 8);
	private double x;
	private double y;
	private boolean toLeft;
	private boolean toBottom;
	private Zone zone;
	public MonstreF(Zone zone) {
		this.zone = zone;
		x = zone.getDebutX() + (zone.getFinX() - zone.getDebutX()) * Math.random();
		y = zone.getDebutY() + (zone.getFinY() - zone.getDebutY()) * Math.random() + 80;

		try {
			image = new Image(new FileInputStream("photosJeu/monstre-f.png"));
